...
HKEY RegOpen(const char *subKey)
{
  ...
  // logTrace("RegOpen hkey ", THE_HKEY, ", subKey ", subKey);
  return hkRes;
}
...
std::string GetRegString(const std::string subKey, const std::string var, const std::string defval)
{
  ...
  if (RegQueryValueEx(hk, var.c_str(), NULL, &type, NULL, &lval) == ERROR_SUCCESS && type == REG_SZ)
  {
...
{
  HKEY hk = RegOpen(subKey);
  // logTrace("PutRegString key ", subKey, ", var ", var, ", val ", val);
  RegSetValueEx(hk, var, 0, REG_SZ, (LPBYTE)val, (DWORD)std::strlen(val) + 1);
  RegCloseKey(hk);
}
