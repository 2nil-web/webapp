...
webapp_set_title(appName);
// To be called with -m option
webapp_show();
webapp_set_icon("app.ico");
...
function conf_load() {
    url.value = getItemOrDefault('url', "https://wiki.space.");
    ...
    userlist.value = getItemOrDefault('userlist', "toto, tata, tutu");
...
async function wiki_rest(dt) {
    ...
    const response = await fetch(
        `${url.value}/rest/api/content/search?cql=contributor+in+(${userlist.value})+and+space+=+${space.value}+and+lastmodified+=+${curr_date}&limit=1000`, {
            headers: new Headers({
                "Authorization": `${login.value}:${psswd.value}`
            }),
        }
    );
